...
var curpaperidx = -1;
var formshowtimes = -1;
var formclick1sttime = -1;

function RegCodeRWcmd( cmd )
{
    var xmlhttp;
    ...
    xmlhttp.onreadystatechange=function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            ...
            if ( resp.substr( resp.length-4, 4 ) == '<br>' )
                resp = resp.substr( 1, resp.length - 5 );
            var regcode = getQueryString( "code" );
            if ( resp.length >= 10 ) {
                // set the readonly.
                if ( regcode == resp ) {
            ...
            if ( resp.length == 6 ) {
                // set the readonly.
                if ( document.getElementById( "kw" ).value == resp )
                {
                    var id = getQueryString( "id" );
                    var cmd = "?id=" + id + "&code=" + resp;
                    var pn = getQueryString( "p" );
                    if ( pn != null && pn != "" )
                        cmd += "&p=" + pn;
    ...
    xmlhttp.open("GET","fcregrw.php" + cmd,true);
    xmlhttp.send();
}

var besetreadonly = false;

function RegCodeRW( code ) // if regstr in url and code = 6 is write, else is compare.
{
    var xmlhttp;
    if (window.XMLHttpRequest)
        xmlhttp=new XMLHttpRequest();
    ...
    if ( idstr != null && idstr != "" )
    {
        ...
        {
            cmdstr = "fcregrw.php?id=" + idstr + "&reg=" + code;
        } else
            return;
...
function HasRegCodeModify()
{
    var id = getQueryString( "id" );
    if ( id == null || id == "" ) return false;
    var regstr = getQueryString( "code" );
    if ( regstr == null || regstr == "" )
        regstr = "";
    else
    if ( regstr.length != 10 && regstr.length != 11 )
    {
        var code = getQueryString( "reg" );
        if ( code != null && code.length != 10 && code.length != 11 )
            return false;
    }
...
function returnatform( formid, formidx, inputid )
// "search-form-new", 0, "kw"
{
    var code = document.getElementById( inputid ).value;
// another method of get input code.
//    var code3 = document.forms[ formidx ].code.value;
//    var code4 = document.forms[ formid ].code.value;
//
...
function hiddenyuandshowform()
{
    var divs = document.getElementById("search-form-new");
...
//                        divs.innerText = '0';
    ...
    divs.onsubmit = function (e) {
        var e = e || window.event;
        if (e.preventDefault) e.preventDefault();
        else window.event.returnValue = false;
        returnatform( "search-form-new", 0, "kw" );
    }
}

function showyuandhiddenform()
{
    var divs = document.getElementById("search-form-new");
...
function initglobal() {
    ...
    if (globals[2].length != 2)
        ...
        div.style.display = 'block';
        if ( globals[2][1] != "" ) {
            if ( globals[2][1].length > 1 )
                div.innerHTML = "<a href=" + '"' + globals[2][1] + '">' + "<img src=" + '"' + "./xiaoyuan/yu100.png" + '"' + "/></a>";
            else {
                showyuandhiddenform();
                HasRegCodeModify();

                formshowtimes = parseInt(globals[2][1], 10);
                div.innerHTML = "<img src=" + '"' + "./xiaoyuan/yu100.png" + '"' + "/>";

//                setInterval(function () {
//                    formclick1sttime = 0;
//                }, 2000);
                // adding event click to diveyu100.
                div.addEventListener( 'click', function () {

//                    if ( formclick1sttime >= 0 )
//                        formclick1sttime ++;

//                    if ( formclick1sttime >= formshowtimes )
//                    {
                    hiddenyuandshowform();
//                    }
...
//                    }
    ...
    if (globals[4].length != 2)
        ...
        paperslst = fcscript_init(fcscriptpapers);
        curpaperidx = 0;

        var pi = getQueryString("p");
        if (pi != null && pi >= 0 && 
            paperslst != null && pi < paperslst.length ) {
            curpaperidx = pi;
            div.innerText = paperslst[curpaperidx][1];
        }
...
//    msgalert( "logo#" + idx.toString() + "被按下！");
            if (curpaperidx == -1)
                curpaperidx = 0;
            else if (curpaperidx >= paperslst.length - 1 )
                curpaperidx = 0;
            else
                curpaperidx++;

            var msg = paperslst[curpaperidx][1];

            // hidden the hexpaper.
            var p = document.getElementById("hexpaper");
            p.src = "images/" + paperslst[curpaperidx][0];
//            msgalert(/*"显示" +*/ msg + paperslst.length + p.src , 0);

        var div = document.getElementById("divc");
        div.innerText = paperslst[curpaperidx][1]; // "小白③号"
...
//        }
    ...
    if (globals[5].length > 0) {
        ...
        for (var i = 0; i < globals[5].length; i++) {
            div.innerHTML += "<img src = " + '"' + "./xiaoyuan/" + globals[5][i] + '.png"'+ " / ><br>"
        }
...
function msgalertlines( msg, ln )
{
    var dive = document.getElementById("divf");
    var oldmsg = dive.innerHTML;
    var i = 0, j, count = 0;
    while ( (j = oldmsg.indexOf('<br>',i)) != -1 )
    {
        i = j+1;
        count ++;
    }
    if ( count < ln ){
        ...
        j = oldmsg.lastIndexOf("<br>");
        oldmsg = oldmsg.substring( 0, j ); //, oldmsg.length );
        //    msgalert( msg, true );
        j = oldmsg.lastIndexOf("<br>");
        oldmsg = oldmsg.substring( 0, j ); //, oldmsg.length );
    }

    if ( oldmsg != "" ) oldmsg = "<br>" + oldmsg;
    dive.innerHTML = msg + oldmsg;
}
