 ...
 * Newest for Tibet Chess road. 2019/12/09.
 */
...
//        chess.y = r * ( g_rsize );// + 30
        chess.x = ( pos.c + 4/2 + 1/2+3/*2*(11-g_col + 1 + 1)*/ )  * g_rsize; //  + r * 30;// + 30 - g_row*15 - ((50-20)/2);
...
function getChessPos(x,y){
    ...
    if ( gIsRectBoard ) {
         ...
         */
        x -= (/* 2*(11-g_col+1+1)*/1 + 4/2 +2)  * g_rsize;
...
function moveBox(box, pos) {
    ...
    if ( gIsRectBoard ) {
        ...
        y = pos.r * ( g_rsize ) - 1;
        x += (  4/2 +/* 2*(11-g_col + 1+ 1) */1/2+3 )  * g_rsize;
        y += ( /*(g_col-g_row)/2 - */1/2 ) * g_rsize;
    } else {
...
//        chess.x = c * g_rsize ;// + g_rsize/2 - g_row*15 - ((50-20)/2);
//        chess.y = r * ( g_rsize );// + 30
        chess.x = ( c + 4/2 + 1/2+3/*2*(11-g_col + 1 + 1) */)  * g_rsize; //  + r * 30;// + 30 - g_row*15 - ((50-20)/2);
    ...
    for (var r = 0; r < g_row; r++) {
        for (var c = 0; c < g_col; c++) {
            ...
            if (c == 0 && r == 0 && first) {
                img = new Image();
                if (gIsRectBoard) {
                    img.src = "images/RECTpaper.png";
                } else
                    img.src = "images/HEXpaper.png";
                img.style.position = 'absolute'
                if (gIsRectBoard) {
                    img.style.top = ( chess.y - (595 - g_rsize * (g_row)) / 2) + "px"
                } else
		...
		if ( paperslst != null && curpaperidx >= 0 &&
            ...
            if (boardrects[i] == '0' || gIsRectBoard) {
                ...
                if ( r == 0 )
                {
                    ...
                    img.src = "images/" + (c > 9 ? 1 : 0).toString() + ".png";
                    img.style.position = 'absolute'
                    img.style.top = ( chess.y - 1 + 9 + label_digi_y ) + "px"
